# -*- coding: utf-8 -*-
"""
Vortex V5.6: budget-matched ablation + repetition diagnostics
...
print("🚀 启动 M4 统一内存... 载入 Vortex V5.6 内存收割者 (Gemma-2-2B)")
...
# 固定预算压缩曲线。
# 这是 online budget controller：在每个 prefix 上限制保留配额，并在剩余 token 不足时强制补齐。
# 它不是离线 top-k oracle，但能让 Vortex/Random/消融组在同一保留预算下公平比较。
RUN_BUDGET_CURVES = True
BUDGET_TARGET_RATES = (0.80, 0.70, 0.60)
BUDGET_CURVE_STRATEGIES = (
    "geo",
    "guard_only",
    "vortex_no_logic",
    "vortex_no_jump",
...
BUDGET_RANDOM_SEEDS = RANDOM_SEEDS

# 报告文件。
EXPERIMENT_NAME = "vortex_v5_6"
REPORT_JSON_PATH = f"{EXPERIMENT_NAME}_reports.json"
REPORT_CSV_PATH = f"{EXPERIMENT_NAME}_reports.csv"
...
# 类别级逻辑质量指标，避免 “无/无法” 这类子串重叠把 string recall 虚高。
LOGIC_QUALITY_CATEGORIES = {
    "negation": ("不", "没", "没有", "无", "未", "非", "并非", "不是", "不能", "不要", "无法"),
    "contrast": ("但", "但是", "却", "然而", "不过", "否则"),
    "causality": ("因为", "所以", "因此", "导致", "如果", "若", "则", "除非"),
    "temporal": ("之前", "之后", "以前", "以后", "直到", "正在", "已经", "曾经", "将要"),
}
...
def cache_offset_snapshot(cache) -> Dict[str, object]:
    offsets: List[int] = []
    ...
    if not offsets:
        return {
            "cache_offset_min": None,
            "cache_offset_max": None,
            "cache_offset_mean": None,
        }
    return {
        "cache_offset_min": min(offsets),
        "cache_offset_max": max(offsets),
        "cache_offset_mean": sum(offsets) / len(offsets),
    }
...
def ngram_repetition_stats(text: str, n: int) -> Dict[str, object]:
    units = similarity_units(text)
    if len(units) < n:
        return {
            f"repeat_{n}gram_rate": 0.0,
            f"repeat_{n}gram_unique": 0,
            f"repeat_{n}gram_total": 0,
            f"repeat_{n}gram_max": "",
            f"repeat_{n}gram_max_count": 0,
        }
    grams = [tuple(units[i:i + n]) for i in range(len(units) - n + 1)]
    counts = Counter(grams)
    repeated_excess = sum(c - 1 for c in counts.values() if c > 1)
    max_gram, max_count = max(counts.items(), key=lambda kv: (kv[1], ''.join(kv[0])))
    return {
        f"repeat_{n}gram_rate": 100.0 * repeated_excess / len(grams),
        f"repeat_{n}gram_unique": len(counts),
        f"repeat_{n}gram_total": len(grams),
        f"repeat_{n}gram_max": ''.join(max_gram),
        f"repeat_{n}gram_max_count": max_count,
    }


def logic_category_recall(candidate_text: str, baseline_text: str) -> Dict[str, object]:
    target_categories: List[str] = []
    hit_categories: List[str] = []
    for category, markers in LOGIC_QUALITY_CATEGORIES.items():
        if any(marker in baseline_text for marker in markers):
    ...
    if not target_categories:
        ...
        recall = 100.0 * len(hit_categories) / len(target_categories)
    return {
        "logic_category_targets": target_categories,
        "logic_category_hits": hit_categories,
        "logic_category_recall": recall,
    }
...
def compute_quality_metrics(candidate_text: str, baseline_text: str, retained_piece_text: str) -> Dict[str, object]:
    ...
    output_logic_cat = logic_category_recall(candidate_text, baseline_text)
    retained_logic_cat = logic_category_recall(retained_piece_text, baseline_text)
    repeat3 = ngram_repetition_stats(candidate_text, 3)
    repeat4 = ngram_repetition_stats(candidate_text, 4)
    ...
    return {
        ...
        "output_logic_category_recall": output_logic_cat["logic_category_recall"],
        "output_logic_category_targets": output_logic_cat["logic_category_targets"],
        "output_logic_category_hits": output_logic_cat["logic_category_hits"],
        "retained_logic_category_recall": retained_logic_cat["logic_category_recall"],
        "retained_logic_category_targets": retained_logic_cat["logic_category_targets"],
        "retained_logic_category_hits": retained_logic_cat["logic_category_hits"],
...
def build_random_k_indices(
    ...
    关键修正：STARTUP_KEEP 是所有策略共享的强制开局保护，所以 random sampling
    必须从 startup 之后的 decision 区间采样。否则 startup token 会被“额外白送”，
    造成 actual_kept > target_keep_count。
    """
    if target_decisions is None or target_keep_count is None:
        return None, None, None

    decision_budget = max(0, int(target_decisions))
    forced_startup = min(STARTUP_KEEP, decision_budget)
    requested_keep = max(0, min(int(target_keep_count), decision_budget))

    # 如果目标预算小于强制开局保护，实际最小保留数只能是 forced_startup。
    keep_budget = max(requested_keep, forced_startup)
    if decision_budget == 0:
        return set(), 0, 0

    tail_decisions = max(0, decision_budget - forced_startup)
    tail_keep = max(0, keep_budget - forced_startup)

    keep_indices: Set[int] = set(range(forced_startup))
    if tail_keep > 0 and tail_decisions > 0:
        tail_keep = min(tail_keep, tail_decisions)
        keep_indices.update(forced_startup + i for i in rng.sample(range(tail_decisions), tail_keep))

    return keep_indices, decision_budget, keep_budget


def apply_online_budget(
    keep: bool,
    important: bool,
    reason: str,
    budget_keep_count: Optional[int],
    budget_decision_budget: Optional[int],
    budget_label: Optional[str],
) -> Tuple[bool, bool, str]:
    """
    对非随机策略施加固定预算，得到 budget-matched ablation。

    这是 online controller，不是离线 top-k oracle：
    - 每个 prefix 只允许使用与目标保留率成比例的 keep quota，避免前半段过度消耗预算。
    - 如果剩余 token 数不足以完成目标 keep 数，则强制保留当前 token，保证无 EOS 情况下 exact K。
    - reason 后缀会标记 budget_force / budget_throttle，避免把预算控制误读为原策略本身贡献。
    """
    if budget_keep_count is None or budget_decision_budget is None:
        return keep, important, reason

    decision_budget = max(0, int(budget_decision_budget))
    if decision_budget <= 0:
        return keep, important, reason

    idx = STATE.processed_decode_tokens  # 当前 decode decision 的 0-based index，update 之前调用。
    if idx >= decision_budget:
        return False, False, f"{reason}_budget_after"

    forced_startup = min(STARTUP_KEEP, decision_budget)
    requested_keep = max(0, min(int(budget_keep_count), decision_budget))
    target_keep = max(requested_keep, forced_startup)

    kept_so_far = STATE.kept_count
    remaining_decisions_after_current = decision_budget - idx - 1
    remaining_need = target_keep - kept_so_far

    # 已经完成预算，后续自然 drop。
    if remaining_need <= 0:
        return False, False, f"{reason}_budget_throttle"

    # 如果不保留当前 token，剩余 token 已经不够补齐预算，则强制保留。
    if remaining_need > remaining_decisions_after_current:
        return True, True, f"{reason}_budget_force"

    # Prefix quota：按进度释放预算，避免自然 keep 策略在开头把额度吃光。
    # STARTUP_KEEP 内部由 decide_keep 统一保护；这里仍让 prefix quota 至少覆盖 startup。 
    prefix_quota = max(
        min(idx + 1, forced_startup),
        int((target_keep * (idx + 1) + decision_budget - 1) // decision_budget),
    )
    if keep and kept_so_far >= prefix_quota:
        return False, False, f"{reason}_budget_throttle"
...
def generate_with_strategy(
    ...
    random_decision_budget: Optional[int] = None,
    budget_keep_count: Optional[int] = None,
    budget_decision_budget: Optional[int] = None,
    ...
    if budget_keep_count is not None and budget_decision_budget is not None:
        print(f"📏 Online budget: {budget_label or ''} keep={budget_keep_count}/{budget_decision_budget}")
    print("-" * 132)
    sys.stdout.flush()
    ...
    for step in range(max_tokens):
        ...
        if not is_prefill and strategy != "random_k":
            keep, important, reason = apply_online_budget(
                keep=keep,
                important=important,
                reason=reason,
                budget_keep_count=budget_keep_count,
                budget_decision_budget=budget_decision_budget,
    ...
    offset_mem = cache_offset_snapshot(cache)
...
def aggregate_reports(reports: List[Dict[str, object]], strategy: str, label: str) -> Optional[Dict[str, object]]:
    ...
    numeric_keys = [
        ...
        "output_logic_category_recall", "retained_logic_category_recall",
        "repeat_3gram_rate", "repeat_4gram_rate", "repeat_3gram_max_count", "repeat_4gram_max_count",
        "cache_offset_min", "cache_offset_max", "cache_offset_mean",
...
def print_summary(reports: List[Dict[str, object]]) -> None:
    ...
    print(
        ...
        f"{'OutEnt':>8} | {'RetEnt':>8} | {'OutLog':>8} | {'RetLog':>8} | {'LogCat':>8} | {'BaseF1':>8} | {'LCS':>8} | {'Rep4':>8}"
    )
    print("-" * 164)
    for r in reports:
        print(
            ...
            f"{fmt_pct(r.get('output_logic_category_recall')):>8} | "
            f"{fmt_pct(r.get('baseline_similarity')):>8} | "
            f"{fmt_pct(r.get('baseline_lcs')):>8} | "
            f"{fmt_pct(r.get('repeat_4gram_rate')):>8}"
        )
    print("=" * 164)
...
def save_reports(detailed_reports: List[Dict[str, object]], summary_reports: List[Dict[str, object]]) -> None:
    payload = {
        "config": {
            ...
            "budget_target_rates": list(BUDGET_TARGET_RATES),
            "budget_curve_strategies": list(BUDGET_CURVE_STRATEGIES),
        },
    ...
    flat_keys = [
        ...
        "output_logic_recall", "retained_logic_recall", "output_logic_category_recall", "retained_logic_category_recall",
        "baseline_similarity", "baseline_lcs", "repeat_3gram_rate", "repeat_4gram_rate",
        "repeat_3gram_max", "repeat_3gram_max_count", "repeat_4gram_max", "repeat_4gram_max_count",
        "budget_label", "budget_keep_count", "budget_decision_budget", "random_keep_budget", "random_decision_budget",
        "cache_offset_min", "cache_offset_max", "cache_offset_mean",
        "base_volume_p50", "base_volume_p75", "base_volume_p90", "base_volume_p95",
        "local_jump_p50", "local_jump_p75", "local_jump_p90", "local_jump_p95",
        "score_p50", "score_p75", "score_p90", "score_p95",
...
def budget_keep_count_for_rate(decision_budget: int, target_rate: float) -> int:
    decision_budget = max(0, int(decision_budget))
    forced_startup = min(STARTUP_KEEP, decision_budget)
    requested = int(round(decision_budget * float(target_rate)))
    return max(forced_startup, min(decision_budget, requested))


def run_budget_curves(prompt: str, baseline_report: Dict[str, object]) -> List[Dict[str, object]]:
    reports: List[Dict[str, object]] = []
    decision_budget = int(baseline_report.get("processed_decode_tokens") or MAX_TOKENS)
    for target_rate in BUDGET_TARGET_RATES:
        keep_budget = budget_keep_count_for_rate(decision_budget, target_rate)
        label = f"@{int(round(target_rate * 100))}"
        print("\n" + "#" * 132)
        print(f"📈 Budget curve {label}: keep={keep_budget}/{decision_budget}")
        print("#" * 132)

        for strategy in BUDGET_CURVE_STRATEGIES:
            r = generate_with_strategy(
                prompt,
                max_tokens=MAX_TOKENS,
                strategy=strategy,
                budget_keep_count=keep_budget,
                budget_decision_budget=decision_budget,
                budget_label=label,
                seed=0,
                print_stream=True,
            )
            r["strategy"] = f"{strategy}{label}"
            r["strategy_name"] = f"{STRATEGY_NAMES[strategy]} {label} budget"
            reports.append(r)
            print("\n" + "=" * 132)

        # 同预算 Random-k 多 seed：作为每条曲线的硬对照。
        for idx, seed in enumerate(BUDGET_RANDOM_SEEDS):
            r = generate_with_strategy(
                prompt,
                max_tokens=MAX_TOKENS,
                strategy="random_k",
                random_keep_budget=keep_budget,
                random_decision_budget=decision_budget,
                seed=seed,
                print_stream=(not PRINT_ONLY_FIRST_RANDOM_STREAM or idx == 0),
            )
            r["strategy"] = f"random_k{label}"
            r["strategy_name"] = f"Random-k {label} mean-source"
            r["budget_label"] = label
            r["budget_keep_count"] = keep_budget
            r["budget_decision_budget"] = decision_budget
            reports.append(r)
            print("\n" + "=" * 132)
    return reports
...
if __name__ == "__main__":
    ...
    if RUN_BUDGET_CURVES:
        budget_reports = run_budget_curves(test_prompt, deterministic_reports[0])

    detailed_reports = deterministic_reports + random_p_reports + random_k_reports + budget_reports
    ...
    # Budget curve rows：deterministic rows 逐条展示；Random-k@budget 做 mean±std 聚合。
    if budget_reports:
        random_budget_groups: Dict[str, List[Dict[str, object]]] = {}
        for r in budget_reports:
            st = str(r.get("strategy"))
            if st.startswith("random_k@"):
                random_budget_groups.setdefault(st, []).append(r)
            else:
                summary_reports.append(r)
        for st, rows in sorted(random_budget_groups.items()):
            label = st.replace("random_k", "")
            agg = aggregate_reports(rows, st + "_mean", f"Random-k {label} mean n={len(rows)}")
            if agg is not None:
                agg["budget_label"] = label
                summary_reports.append(agg)

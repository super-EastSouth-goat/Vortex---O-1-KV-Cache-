# -*- coding: utf-8 -*-
"""
Vortex V5.2: Geometry Probe + Jieba Lexical Prior + Post-Hit Local Fuse

V5.2 相对 V5.1 的四处小改动：
1. RECENT_KEEP 改名为 FUSE_KEEP：
   - 语义上明确为 post-hit local fuse，而不是 strict recent window。
2. Prefill 不再更新 prev_actual_token_id：
   - Jieba 词法先验只从真正的 decode 相邻 token 开始。
3. 增加 reason_counts 与 logic_reason_counts：
   - 统计普通 token 与逻辑词分别是被 jump、logic guard、Jieba、volume 还是 fuse 留住。
4. 新增 vortex_no_logic 消融：
   - 保留几何探针、Jieba 与 post-hit local fuse，但关闭 logic guard，用于验证 JUMP_THRESHOLD 理论。

注意：
- 当前 offset 回退法是在“当前 token 已写入 KV cache 后”决定是否保留当前 token。
- FUSE_KEEP=4 在这里是“重要 token 点燃的 post-hit local fuse”，不是完整 H2O 式滑动窗口。
- teacher-forced logprob gap 留作下一版；这版先把轻量质量指标接入 ablation summary。
"""
...
print("🚀 启动 M4 统一内存... 载入 Vortex V5.2 内存收割者 (Gemma-2-2B)")
...
# Global switch for the vortex_no_logic ablation.
...
class VortexRuntime:
    ...
    # V5/V5.2 post-hit local fuse.
    local_fuse_remaining: int = 0
    ...
    # Decision attribution counters for ablation evidence.
...
def decide_keep(
    ...
    - important: 是否由核心规则命中，而不是仅由 post-hit local fuse 保护。
    - reason: 调试原因。
    """
    if is_prefill:
    ...
    if strategy in {"vortex", "vortex_no_logic"}:
        # 局部跳跃“破盾”：如果语义突然跳变，不继续沿用上一段的 post-hit local fuse。
        # 但当前 token 本身会因为 jump_hit 被视为重要 token，从而重新点燃保险丝。
        if STATE.jump_hit:
            STATE.local_fuse_remaining = 0

        fuse_keep = STATE.local_fuse_remaining > 0
        important = (
        ...
        if important:
            prefix = "vortex_no_logic" if strategy == "vortex_no_logic" else "vortex"
            if STATE.jump_hit:
                reason = f"{prefix}_jump"
            elif STATE.lexical_hit:
                reason = f"{prefix}_jieba"
            elif STATE.logic_hit:
                reason = f"{prefix}_logic"
            elif STATE.boundary_hit:
                reason = f"{prefix}_boundary"
            else:
                reason = f"{prefix}_volume"
            return True, True, reason

        if fuse_keep:
            return True, False, "post_hit_fuse"

        low_reason = "vortex_no_logic_low" if strategy == "vortex_no_logic" else "vortex_low"
        return False, False, low_reason

    raise ValueError(f"未知策略: {strategy}")


def update_after_decision(is_prefill: bool, keep: bool, important: bool, strategy: str) -> None:
    """决策之后统一更新几何参考、词法参考、post-hit local fuse 与统计。"""
    if not is_prefill:
        ...
        if strategy in {"vortex", "vortex_no_logic"}:
            # 如果只是被保险丝保护，消耗一格保险丝。
            if keep and not important and STATE.local_fuse_remaining > 0:
                STATE.local_fuse_remaining -= 1

            # 只有核心命中才重新点燃局部保险丝。
            if important and keep:
                STATE.local_fuse_remaining = max(STATE.local_fuse_remaining, max(FUSE_KEEP - 1, 0))

    # 真实相邻 token 参考永远更新，不管当前 token 有没有幸存。
    # 这是 Jieba 词法系统和几何幸存系统解耦的关键。
    if (not is_prefill) and STATE.current_input_id is not None:
...
STRATEGY_NAMES = {
    ...
    "vortex_no_logic": "Vortex-no-logic 消融模式",
    "vortex": "Vortex V5.2 大一统模式",
}


def strategy_needs_probe(strategy: str) -> bool:
    # V5.2 改动二：Lexical-only 不挂 Q 探针，保证消融干净。
    return strategy in {"geo", "vortex", "vortex_no_logic"}
...
def generate_with_strategy(
    ...
    global ENABLE_LOGIC_GUARD
    ENABLE_LOGIC_GUARD = strategy != "vortex_no_logic"
    ...
    for step in range(max_tokens):
        ...
        if not is_prefill:
            ...
            if normalize_piece(STATE.current_text) in LOGIC_GUARD_STRINGS:
        ...
        if debug_tokens:
            ...
            print(
                ...
                f"logic={STATE.logic_hit} fuse={STATE.local_fuse_remaining} "
                f"reason={reason}]",
                flush=True,
            )
        else:
...
def fmt_counter(counter_obj: object, max_items: int = 8) -> str:
    if not counter_obj:
        return "n/a"
    try:
        items = Counter(counter_obj).most_common(max_items)
    except Exception:
        return "n/a"
    if not items:
        return "n/a"
...
def print_summary(reports: List[Dict[str, object]]) -> None:
    ...
    print("\nReason attribution:")
    for r in reports:
        if r.get("strategy") in {"vortex_no_logic", "vortex"}:
            print(f"- {r['strategy_name']} reasons: {fmt_counter(r.get('reason_counts'))}")
            print(f"  logic-token reasons: {fmt_counter(r.get('logic_reason_counts'))}")
...
if __name__ == "__main__":
    ...
    # D. Vortex-no-logic：几何探针 + Jieba + post-hit local fuse，但关闭 logic guard。
    reports.append(generate_with_strategy(test_prompt, max_tokens=MAX_TOKENS, strategy="vortex_no_logic"))

    print("\n" + "=" * 88)

    # E. Vortex V5.2：几何探针 + Jieba + 逻辑保护 + post-hit local fuse。
    vortex_report = generate_with_strategy(test_prompt, max_tokens=MAX_TOKENS, strategy="vortex")
    reports.append(vortex_report)

    print("\n" + "=" * 88)

    # F. Random-p：概率保留率对齐 Vortex。
